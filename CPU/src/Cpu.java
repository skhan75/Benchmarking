...
public class Cpu {
	...
	public static void main(String[] args) throws InterruptedException{
		...
		System.out.println("\nEnter the number of concurrent Threads you want to run (1,2,4)");
		Scanner in = new Scanner(System.in);
		totalThreads = in.nextInt();
		
		cpuInfo();
		System.out.println("Running "+totalThreads+" Concurrent threads..");
				
		for(int i = 1; i <= totalThreads; i++){
			CpuLoader cldr = new CpuLoader();
			cldr.noOfThreads = i;
			Thread t = new Thread(cldr);
			t.start();
			t.join();
